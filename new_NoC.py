...
parser = argparse.ArgumentParser(description='Detailed options for MODEL config.')
...
#here put the time to start the simulation.
#However, because of the other configurations, please allow one minute or more gap between start and end time.
#Otherwise, it will conflict with the message generators and throw an error.
...
# 2) Noise Agents
# Change here for the number of agents. Can configure anything from 1 onwards
num_noise = 5
...
kernel = Kernel("MODEL Kernel", random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2 ** 32,
                                                                                                  dtype='uint64')))
...
# Can configure distances here - like moscow to saint-petersberg.
